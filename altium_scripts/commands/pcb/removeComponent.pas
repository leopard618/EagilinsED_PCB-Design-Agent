 ...
 * Removes a component from the PCB
 * Command: remove_component
 * Parameters: {"component_name": "U1"}
 *}
...
Function ExecuteRemoveComponent(CommandJson: String): Boolean;
Var
    PCB: IPCB_Board;
    Component: IPCB_Component;
    CompName: String;
Begin
    ...
    If CompName = '' Then
    Begin
        ShowMessage('ERROR: Missing parameter: component_name');
        Exit;
    End;
    ...
    Try
        ...
        Result := True;
        ShowMessage('SUCCESS: Component "' + CompName + '" removed');
    Except
        ShowMessage('ERROR: Failed to remove component');
    End;
End;
...
Begin
    ...
    Try
        ...
        CommandStart := Pos('"remove_component"', LowerCase(FileContent));
        If CommandStart = 0 Then
        Begin
            ShowMessage('No remove_component command found in ' + FileName);
            Exit;
        End;
        
        CommandJson := Copy(FileContent, Max(1, CommandStart - 200), Min(2000, Length(FileContent) - Max(1, CommandStart - 200) + 200));
        
        Success := ExecuteRemoveComponent(CommandJson);
